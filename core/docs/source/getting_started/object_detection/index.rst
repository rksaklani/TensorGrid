...
This step-by-step guide will walk you through a complete object detection workflow using TensorGrid. You'll learn how to:

- Load and explore detection datasets with TensorGrid
- Add object detection predictions using pre-trained models
- Find and analyze detection mistakes using TensorGrid Brain
...
1. **Loading Detection Datasets** - Load detection datasets into TensorGrid using both built-in datasets from the zoo and custom datasets
...
3. **Finding Detection Mistakes** - Use TensorGrid Brain to identify detection mistakes including erroneous boxes, class mistakes, and overlapping detections
4. **Evaluating Detections** - Perform comprehensive evaluation of your detection model using TensorGrid's evaluation API and analyze best and worst performing samples
...
This tutorial is designed for computer vision practitioners and data scientists who want to master object detection workflows using TensorGrid. Whether you're new to computer vision or experienced with other tools, you'll learn how to leverage TensorGrid's powerful capabilities for detection dataset curation, model evaluation, and visual analysis.
...
**TensorGrid Model Zoo Integration**

We'll also explore how to use pre-trained models from TensorGrid's Model Zoo, which provides easy access to state-of-the-art detection models with standardized interfaces for inference and evaluation.
...
1. **Data Management** - Using TensorGrid to load, organize, and explore detection datasets with rich metadata and annotations

2. **Model Integration** - Seamlessly adding predictions from multiple detection models and comparing their performance

3. **Quality Analysis** - Leveraging TensorGrid Brain to automatically identify potential annotation errors and model failure cases
...
Click **Next** to start with the first step: Loading Detection Datasets with TensorGrid.

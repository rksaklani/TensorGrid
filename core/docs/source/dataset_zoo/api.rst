...
.. tabs::
  ...
  .. group-tab:: Python
    ...
    The Dataset Zoo is accessible via the :mod:`tensorgrid.zoo` package.

  .. group-tab:: CLI

    The :ref:`tensorgrid zoo datasets <cli-fiftyone-zoo-datasets>` command
    provides convenient utilities for working with datasets in the TensorGrid
    Dataset Zoo.
...
.. tabs::
  ...
  .. group-tab:: Python
    ...
    :meth:`list_downloaded_zoo_datasets() <tensorgrid.zoo.datasets.list_downloaded_zoo_datasets>`:
    ...
    .. code-block:: text

        {
            ...
            'cifar10': (
                '~/fiftyone/cifar10',
                <tensorgrid.zoo.datasets.ZooDatasetInfo object at 0x141a63048>,
            ),
            'kitti': (
                '~/fiftyone/kitti',
                <tensorgrid.zoo.datasets.ZooDatasetInfo object at 0x141a62940>,
            ),
            ...
        }
  ...
  .. group-tab:: CLI
    ...
    :ref:`tensorgrid zoo datasets list <cli-fiftyone-zoo-datasets-list>` command.
    ...
    TensorGrid without requiring an ML backend.
...
.. tabs::
  ...
  .. group-tab:: Python
    ...
    :class:`ZooDataset <tensorgrid.zoo.datasets.ZooDataset>` subclass, which
    ...
    :meth:`get_zoo_dataset() <tensorgrid.zoo.datasets.get_zoo_dataset>` method.
    ...
    :class:`ZooDatasetInfo <tensorgrid.zoo.datasets.ZooDatasetInfo>` instance is
    ...
    You can load the
    :class:`ZooDatasetInfo <tensorgrid.zoo.datasets.ZooDatasetInfo>`
    instance for a downloaded dataset via
    :meth:`load_zoo_dataset_info() <tensorgrid.zoo.datasets.load_zoo_dataset_info>`.
    ...
    .. code-block:: text
        ...
        ***** Dataset info *****
        {
            "name": "cifar10",
            "zoo_dataset": "tensorgrid.zoo.datasets.torch.CIFAR10Dataset",
            "dataset_type": "tensorgrid.types.dataset_types.ImageClassificationDataset",
            "num_samples": 10000,
            "downloaded_splits": {
                "test": {
                    "split": "test",
  ...
  .. group-tab:: CLI
    ...
    not) via the
    :ref:`tensorgrid zoo datasets info <cli-fiftyone-zoo-datasets-info>` command.

    For example, you can view information about the CIFAR-10 dataset:

    .. code-block:: shell

        tensorgrid zoo datasets info cifar10
...
.. tabs::
  ...
  .. group-tab:: Python
    ...
    .. code-block:: text
        ...
        Writing samples to '~/fiftyone/cifar10/train' in 'tensorgrid.types.dataset_types.ImageClassificationDataset' format...
  ...
  .. group-tab:: CLI
    ...
    .. code-block:: shell
        ...
        tensorgrid zoo datasets download cifar10 --splits test
    ...
    .. code-block:: text
        ...
        Extracting ~/fiftyone/cifar10/tmp-download/cifar-10-python.tar.gz to ~/fiftyone/cifar10/tmp-download
        Writing samples to '~/fiftyone/cifar10/test' in 'tensorgrid.types.dataset_types.ImageClassificationDataset' format...
...
.. tabs::
  ...
  .. group-tab:: Python
    ...
    :meth:`load_zoo_dataset() <tensorgrid.zoo.datasets.load_zoo_dataset>`.
    ...
    :meth:`load_zoo_dataset() <tensorgrid.zoo.datasets.load_zoo_dataset>` to
  ...
  .. group-tab:: CLI
    ...
    a TensorGrid dataset via the
    :ref:`tensorgrid zoo datasets load <cli-fiftyone-zoo-datasets-load>`
    command.
    ...
    .. code-block:: shell
        ...
        tensorgrid zoo datasets load cifar10 --splits test
...
:class:`BDD100K <tensorgrid.zoo.datasets.base.BDD100KDataset>`
and :class:`Cityscapes <tensorgrid.zoo.datasets.base.CityscapesDataset>` require
that you create accounts on a website and manually download the source files.
In such cases, the :class:`ZooDataset <tensorgrid.zoo.datasets.ZooDataset>` class
...
You can load these datasets into TensorGrid by first calling
:meth:`download_zoo_dataset() <tensorgrid.zoo.datasets.download_zoo_dataset>`
with the appropriate keyword arguments (which are passed to the underlying
:class:`ZooDataset <tensorgrid.zoo.datasets.ZooDataset>` constructor) to wrangle
the raw download into TensorGrid format, and then calling
:meth:`load_zoo_dataset() <tensorgrid.zoo.datasets.load_zoo_dataset>` or using
:ref:`tensorgrid zoo datasets load <cli-fiftyone-zoo-datasets-load>` to load the
dataset into TensorGrid.
...
``tensorgrid.config.dataset_zoo_dir`` corresponding to their names.

You can customize this backend by modifying the `dataset_zoo_dir` setting
of your :ref:`TensorGrid config <configuring-fiftyone>`.
...
.. tabs::
  ...
  .. group-tab:: CLI
    ...
    .. code-block:: shell
        ...
        tensorgrid zoo datasets delete cifar10 --splits test
...
automatically become accessible to you when you update your TensorGrid package.

.. note::

    TensorGrid is open source! You are welcome to contribute datasets to the
...
work with these datasets via the :mod:`tensorgrid.zoo.datasets` package and the
...
the format below to tell TensorGrid about the dataset. For example, the manifest
...
.. code-block:: json
    ...
    {
        "custom": {
            "quickstart-copy": "tensorgrid.zoo.datasets.base.QuickstartDataset"
        }
    }
...
:ref:`tensorgrid zoo datasets list <cli-fiftyone-zoo-datasets-list>` listing in
which the dataset is annotated; ``quickstart-copy`` is the name of the new
dataset; and ``tensorgrid.zoo.datasets.base.QuickstartDataset`` is the
fully-qualified class name of the
:class:`ZooDataset class <tensorgrid.zoo.datasets.ZooDataset>` for the dataset,
which specifies how to download and load the dataset into TensorGrid. This class
can be defined anywhere that is importable at runtime in your environment.

Finally, expose your new dataset(s) to TensorGrid by adding your manifest to the
...
.. code-block:: shell
    ...
    # Load custom dataset into TensorGrid
    tensorgrid zoo datasets load quickstart-copy
...
Behind the scenes, TensorGrid uses either
...
For datasets that require an ML backend, TensorGrid will use whichever ML backend
...
`fo.config.default_ml_backend` setting in your TensorGrid config.

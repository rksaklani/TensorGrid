...
easy to use Pgvector on your computer vision data directly from TensorGrid!

Follow these :ref:`simple instructions <pgvector-setup>` to get started
using Pgvector + TensorGrid.

TensorGrid provides an API to create Pgvector indexes, upload vectors, and
...
Alternatively, you can permanently configure TensorGrid to use the pgvector
...
by TensorGrid whenever a connection to pgvector is made.
...
a TensorGrid dataset using the pgvector backend.

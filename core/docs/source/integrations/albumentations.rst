...
TensorGrid datasets, enabling you to visualize Albumentations augmentations
and test their effects on your data directly within the TensorGrid App!

This integration takes the form of a :ref:`TensorGrid plugin <using-plugins>`, 
which is easy to install and can be used entirely via a convenient graphical
interface.

With the TensorGrid Albumentations plugin, you can transform any and all labels 
...
Before we get started, let's take a look at the main features of the TensorGrid
Albumentations integration.
...
The TensorGrid Albumentations plugin currently supports all but the following transformations:
...
The TensorGrid Albumentations plugin provides the following functionality:

- Apply Albumentations transformations to your dataset, your current view, or selected samples
- Visualize the effects of these transformations directly within the TensorGrid App
...
To get started, first make sure you have TensorGrid and Albumentations installed:

.. code-block:: bash

    $ pip install -U tensorgrid albumentations

Next, install the
`TensorGrid Albumentations plugin <https://github.com/jacobmarks/fiftyone-albumentations-plugin>`_:

.. code-block:: bash

    $ tensorgrid plugins download https://github.com/jacobmarks/fiftyone-albumentations-plugin

.. note::

    If you have the 
    `@voxel51/plugins plugin <https://github.com/rksaklani/TensorGrid-plugins/tree/main/plugins/plugins>`_
...
.. note::
    ...
    examples to get you started, using TensorGrid's
...
transformations directly within the TensorGrid App. All augmented samples will be
...
:meth:`match_tags() <tensorgrid.core.collections.SampleCollection.match_tags>` method:
...
.. note::
    ...
    For all samples added to the dataset by the TensorGrid Albumentations plugin,
...
By default all augmentations are temporary, as the TensorGrid Albumentations
...
TensorGrid Albumentations plugin, this information is captured and stored using
TensorGrid's :ref:`custom runs <managing-custom-runs>`. This means that you can
easily access the information about the last applied transformation.

In the TensorGrid App, you can use the

...
TensorGrid provides an API to upload data, define label schemas,
...
The basic workflow to use V7 to add or edit labels on your TensorGrid datasets is
as follows:
...
In order to use the V7 backend, you must install the ``darwin_tensorgrid`` Python
package:

.. code-block:: shell

    pip install darwin_tensorgrid

and register the ``darwin`` backend with TensorGrid, which you can do either by
setting the following environment variables:

.. code-block:: shell

    export FIFTYONE_ANNOTATION_BACKENDS=*,darwin
    export FIFTYONE_DARWIN_CONFIG_CLS=darwin_tensorgrid.DarwinBackendConfig
...
.. code-block:: text

    {
        "backends": {
            "darwin": {
                "config_cls": "darwin_tensorgrid.DarwinBackendConfig",
                "api_key": "XXXXXXXXX"
            }
        }
    }
...
Alternatively, you can permanently configure TensorGrid to use the V7 backend by
...
by TensorGrid whenever a connection to V7 is made.
...
TensorGrid dataset using the V7 backend.

...
// Stub @tensorgrid/lighter and ./selectors to keep the createNew import graph
// hermetic — the real lighter package and selectors transitively pull in
// @tensorgrid/state → analytics → plotly, which breaks in node environments.
...
vi.mock("@tensorgrid/utilities", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@tensorgrid/utilities")>();
  return {
    ...actual,

...
import { viewsAreEqual } from "@tensorgrid/state";
import { NotFoundError, Resource, isNotebook } from "@tensorgrid/utilities";
...
interface TensorGridLocation extends Location {
  state: LocationState<OperationType>;
}

export interface Entry<T extends OperationType> extends TensorGridLocation {
  component: Route<T>;
...
export const createRouter = <T extends OperationType>(
      ...
      try {
        nextCurrentEntryResource =
          ...
          getEntryResource({
            environment,
            routes,
            location: location as TensorGridLocation,
  ...
  const cleanup = history.listen(({ location, action }) => {
    ...
    update(location as TensorGridLocation, action);
  });
  ...
  const context: RoutingContext<T> = {
    ...
    get location() {
      return history.location as TensorGridLocation;
    },
    ...
    load(hard = false) {
      if (!currentEntryResource || hard) {
        currentEntryResource = getEntryResource({
          ...
          hard,
          handleError,
          location: history.location as TensorGridLocation,
          routes,
        });
        update(
          history.location as TensorGridLocation,
          undefined,
...
const makeGetEntryResource = <T extends OperationType>() => {
  let currentLocation: TensorGridLocation;
  let currentResource: Resource<Entry<T>>;

  const isReusable = (location: TensorGridLocation) => {

...
import {
  ...
  iconContainer,
  useTheme,
} from "@tensorgrid/components";
import { isNotebook } from "@tensorgrid/state";
import { styles } from "@tensorgrid/utilities";
...
const remoteSnippet = `import tensorgrid as tg

# Load your TensorGrid dataset
dataset = tg.load_dataset(...)

# Launch a remote App instance that you'll connect to from your local machine
session = tg.launch_app(dataset, remote=True, port=XXXX)
`;

const LocalInstructions = () => {
  const localSnippet = `import tensorgrid as tg

# Load your TensorGrid dataset
dataset = tg.load_dataset(...)

# Launch the app
session = tg.launch_app(dataset, port=${port})
`;
  return (
    <>
...
tg app connect --destination [<username>@]<hostname> \\
...
const Setup = () => {
  ...
  return (
    <div data-cy="setup-page">
      ...
      <SetupWrapper>
        <SetupContainer>
          <Title>Welcome to TensorGrid!</Title>

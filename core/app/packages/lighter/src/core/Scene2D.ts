...
import { Action, CommandContextManager } from "@tensorgrid/commands";
import {
  EventDispatcher,
  EventHandler,
  clearChannel,
  getEventBus,
} from "@tensorgrid/events";
...
export class Scene2D {
  ...
  /**
   * Creates a style for an overlay using TensorGrid's color scheme system.
  ...
  private createOverlayStyle(overlay: BaseOverlay): DrawStyle | null {
    ...
    // Use TensorGrid color scheme if available, otherwise fallback to simple ID-based color

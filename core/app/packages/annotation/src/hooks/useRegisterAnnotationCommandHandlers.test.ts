...
vi.mock("@tensorgrid/annotation", () => ({
  useAnnotationEventBus: vi.fn(),
  useDeleteLabel: vi.fn(),
}));

vi.mock("@tensorgrid/command-bus", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@tensorgrid/command-bus")>();
  return {
    ...actual,
...
import { useAnnotationEventBus, useDeleteLabel } from "@tensorgrid/annotation";
import { useRegisterCommandHandler } from "@tensorgrid/command-bus";
import { useRegisterAnnotationCommandHandlers } from "./useRegisterAnnotationCommandHandlers";
import type { LabelProxy } from "../deltas";
import type { Field } from "@tensorgrid/utilities";

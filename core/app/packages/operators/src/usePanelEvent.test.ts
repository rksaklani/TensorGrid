import { PanelEventError } from "@tensorgrid/utilities";
...
describe("usePendingPanelEventError", () => {
  it("should throw an error if there is a pending error", () => {
    const { result } = renderHook(() => usePendingPanelEventError(), {
      wrapper: TestWrapper,
    });
...
describe("usePanelEvent", () => {
  it("should return a function", () => {
    const { result } = renderHook(() => usePanelEvent(), {
      wrapper: TestWrapper,
    });

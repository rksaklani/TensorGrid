...
import tensorgrid.zoo.models as fozm
...
def _patch_super_gradients_urls():
    """Patches SuperGradients pretrained model URLs to use the current hosting.

    SuperGradients model weights were originally hosted at
    ``https://sghub.deci.ai`` but have been migrated to
    ``https://sg-hub-nv.s3.amazonaws.com``. This function updates the URLs in
    the pretrained models registry and patches the weight loading function to
    handle the new URL format.

    See https://github.com/Deci-AI/super-gradients/issues/2064 for details.
    """
    try:
        ...
        if hasattr(pretrained_models, "MODEL_URLS"):
            ...
            for key, value in pretrained_models.MODEL_URLS.items():
                if isinstance(value, str) and value.startswith(old_domain):
                    ...
                    patched_count += 1
                    logger.debug(
                        f"Patched URL for {key}: {value} -> {new_url}"
                    )

            if patched_count > 0:
                logger.info(
                    f"Patched {patched_count} super-gradients model URLs to use AWS S3"
                )
    ...
    try:
        ...
        def patched_load_pretrained_weights(
            model, architecture, pretrained_weights
        ):
            ...
            from super_gradients.training.utils.checkpoint_utils import (
                ...
                get_local_rank,
                _load_weights,
                _maybe_load_preprocessing_params,
            )
        ...
        checkpoint_utils.load_pretrained_weights = (
            patched_load_pretrained_weights
        )
        model_factory.load_pretrained_weights = patched_load_pretrained_weights
        checkpoint_utils._sg_patched = True
        logger.debug(
            "Patched super-gradients checkpoint_utils for AWS S3 URL compatibility"
        )
    except Exception as e:
        logger.warning(
            f"Failed to patch super-gradients checkpoint_utils: {e}"
        )

...
import tensorgrid.core.config as foc
import tensorgrid.core.labels as fol
import tensorgrid.core.models as fom
import tensorgrid.core.odm as foo
...
class FiftyOneTorchDataset(Dataset):
    ...
    def worker_init(worker_id):
        ...
        """
        import tensorgrid.core.odm.database as food
...
def get_samples_dict_for_get_item(samples, field_mapping, skip_failures=False):
    """Creates a dictionary to use as input to :class:`fiftyone.utils.torch.GetItem` call method.

    Args:
        samples: an iterable of fiftyone samples
        field_mapping: a user-supplied dict mapping keys in
            :class:`GetItem.required_keys` to field names of their dataset that contain
            the required values.
        skip_failures (False): If true, skips failures instead of raising an error.

    Returns:
        a list of dictionary mapping :class:`GetItem.required_keys` to sample field values
    """
    batch = []
...
def get_model_inputs_from_get_item(samples, get_item):
    """Generate model inputs using a :class:`fiftyone.utils.torch.GetItem`.

    Args:
        samples: an iterable of fiftyone samples
        get_item: an instance of a :class:`fiftyone.utils.torch.GetItem`

    Returns:
        a list of dictionary generated by calling :class:`fiftyone.utils.torch.GetItem`
    """
    sample_dicts = get_samples_dict_for_get_item(
        samples, get_item.field_mapping
    )
    return [get_item(sample_dict) for sample_dict in sample_dicts]

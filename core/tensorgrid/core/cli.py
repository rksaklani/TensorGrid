...
class TensorGridCommand(Command):
...
class AppDebugCommand(Command):
    ...
    @staticmethod
    def execute(parser, args):
        import tensorgrid.server.main as fsm
...
def _launch_delegated_local(monitor=False, monitor_interval=60):
    from tensorgrid.core.session.session import _WELCOME_MESSAGE
...
def _parse_paging(sort_by=None, reverse=None, limit=None):
    from tensorgrid.factory import DelegatedOperationPagingParams
...
def _parse_reverse(reverse):
    from tensorgrid.factory import SortDirection
...
def _cleanup_orphan_delegated(dry_run=False):
    from tensorgrid.core.odm import get_db_conn
...
def main():
    ...
    parser = _register_main_command(TensorGridCommand, version=foc.VERSION_LONG)

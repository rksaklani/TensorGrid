...
from tensorgrid.server.app import app
from tensorgrid.server.events import set_port
...
def start_server(
    ...
    # Hypercorn's file reloader forks and can SIGSEGV when launched as a managed
    # subprocess (service/main.py with stdin=PIPE). Only enable for direct TTY runs.
    config.use_reloader = foc.DEV_INSTALL and sys.stdin.isatty()
